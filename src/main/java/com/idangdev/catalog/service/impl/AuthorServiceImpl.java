...
import com.idangdev.catalog.domain.Address;
...
import com.idangdev.catalog.exception.ResourceNotFoundException;
import com.idangdev.catalog.repository.AuthorRepository;
...
public class AuthorServiceImpl implements AuthorService{
	...
	public void updateAuthor(String authorId, AuthorUpdateRequestDTO dto) {
		...
		Map<Long, Address> addressMap = author.getAddresses().stream().map(a->a)
				.collect(Collectors.toMap(Address::getId, Function.identity()));	// Address::getId itu method reference, kan harusnya pake lambda t -> t.getId(), nah bisa di sederhanakan langsung pake method reference Address::getId. sedangkan untuk Function.identity() itu maksudnya adalah object dari Addressnya
		List<Address> addresses = dto.getAddresses().stream().map(a->{
	...
	public void deleteAuthor(String authorId) {
		...
		//2. delete
		//or
		//1. delete langsung (hard delete)
//		authorRepository.deleteById(authorId);		// ini akan hard delete jika tidak menggunakan fitur soft delete dari hibernate @Where & @SQLDelete
...
//		authorRepository.save(author);
		...
		//SOFT DELETE WITH HIBERNATE FITUR
//		authorRepository.deleteById(authorId);		// harus menggunakan @Where & @SQLDelete pada entity AUTHOR
		
		//SOFT DELETE WITH SECURE ID
